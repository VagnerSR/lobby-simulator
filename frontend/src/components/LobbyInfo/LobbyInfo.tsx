    ...
    <div className="flex justify-center flex-col">
      <div className="mt-10">
        <div className="flex justify-start md:justify-between gap-12">
          <LButton text="Leave Lobby" onClickFunc={handleLeaveLobby} />

          {lobbysJson.map((lobby) => {
            if (lobby.id !== lobbyId) return;
            return (
              <h2 className="text-gray-300 text-xl pt-1" key={lobby.id}>
                {lobby.name}
              </h2>
            );
          })}
      ...
      {getLength(lobbyInfo!, lobbyId!) >= 6 ? (
        ...
        <LookingForUsers usersInLobby={getLength(lobbyInfo!, lobbyId!)} />
      )}
    </div>
  );
}
